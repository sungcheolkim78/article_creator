...
def demo_enhanced_article_generation():
    ...
    try:
        enhanced_result = enhanced_generator.forward(
            topic=topic, language="English", use_react=True
        )
        ...
        print(
            f"   Research summary length: {len(enhanced_result.research_summary)} chars"
        )
    ...
    try:
        ...
        print(
            f"   Research phase actions: {len(react_results['research_phase']['actions_taken'])}"
        )
        print(
            f"   Synthesis phase actions: {len(react_results['synthesis_phase']['actions_taken'])}"
        )
        ...
        with open(output_dir / "react_research_analysis.md", "w") as f:
            ...
            f.write(
                f"**Strategy:** {react_results['enhanced_reasoning'].research_strategy}\n\n"
            )
            f.write(
                f"**Action Plan:** {react_results['enhanced_reasoning'].action_plan}\n\n"
            )
    ...
    for i, goal in enumerate(goals, 1):
        ...
        try:
            ...
            for action in result["actions_taken"][-2:]:  # Show last 2 actions
                print(
                    f"   Last action: {action['action']} - {action['thought'][:100]}..."
                )
