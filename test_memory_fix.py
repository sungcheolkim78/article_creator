...
def test_memory_storage():
    ...
    relevant_results = agent.memory.get_relevant_search_results(test_query, max_results=1)
...
def test_react_with_memory():
    ...
    try:
        ...
        print(f"   - Memory Size: {result.get('memory_summary', {}).get('memory_size', 0)}")
        
        # Show actions taken
        for action in result.get('actions_taken', []):
            print(f"   - Action {action['iteration']}: {action['action']} - Success: {action['result'].get('success', False)}")
...
def main():
    ...
    print("This script tests the fixes for SearchResult object handling and no-results scenarios.")
