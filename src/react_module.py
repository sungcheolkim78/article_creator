...
class Memory:
    ...
    def get_relevant_search_results(self, query: str, max_results: int = 5) -> List[Dict]:
        ...
        for stored_query_hash, stored_result in self.memory_store["search_results"].items():
        ...
        relevant_results.sort(key=lambda x: (
            len(set(query_lower.split()).intersection(set(x["query"].lower().split()))),
            x["timestamp"]
        ), reverse=True)
    ...
    def get_recent_insights(self, category: str = None, max_insights: int = 5) -> List[str]:
    ...
    def get_memory_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_research_findings": sum(len(findings) for findings in self.memory_store["research_findings"].values()),
    ...
    def _manage_memory_size(self):
        ...
        for category, items in self.memory_store.items():
            if isinstance(items, dict):
                ...
                sorted_items = sorted(items.items(), key=lambda x: x[1].get("access_count", 0))
...
class ReACTAgent(dspy.Module):
    ...
    def forward(self, goal: str, max_iterations: Optional[int] = None) -> Dict[str, Any]:
        ...
        for iteration in range(max_iter):
            ...
            self._store_action_in_memory(react_output.action, react_output.action_input, action_result)
            ...
            if (react_output.action.lower() == "search" and 
                action_result and not action_result.get("success", True)):
                logger.warning(f"❌ Search failed: {action_result.get('error', 'Unknown error')}")
                if action_result.get("broader_query"):
                    logger.warning(f"💡 Suggestion: Try a broader topic or different keywords")
    ...
    def _get_memory_context(self, goal: str, previous_actions: str) -> str:
        ...
        if relevant_searches:
            ...
            for search in relevant_searches:
                ...
                if search.get('metadata', {}).get('broader_query'):
                    context_parts.append(f"  Broader query used: {search['metadata']['broader_query']}")
        ...
        if relevant_findings:
            ...
            for finding in relevant_findings[:2]:  # Limit to 2 findings
                context_parts.append(f"- {finding['finding'].get('answer', 'Finding available')[:100]}...")
    ...
    def _store_action_in_memory(self, action: str, action_input: str, result: Dict[str, Any]):
        ...
        if action == "search":
            ...
            for search_result in search_results:
                if hasattr(search_result, 'url'):  # Check if it's a SearchResult object
                    results_dict.append({
                        ...
                        "published_time": getattr(search_result, 'published_time', None),
                        "extra_snippets": getattr(search_result, 'extra_snippets', [])
                    })
            ...
            for search_result in search_results:
                if hasattr(search_result, 'url') and search_result.url:  # SearchResult object
                ...
                elif isinstance(search_result, dict) and search_result.get("url"):  # Dictionary
                    self.memory.add_source(
                        url=search_result["url"],
                        source_info={
            ...
            self.memory.add_insight(
                insight=result.get("key_insights", ""),
                category="analysis"
            )
            
            # Store analysis context
            self.memory.add_context(
                context=f"Analysis of: {action_input[:100]}...",
                relevance_score=0.8
            )
    ...
    def _perform_search(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results:
                ...
                logger.warning(f"⚠️  No results found for '{search_query}'. Trying broader search: '{broader_query}'")
                
                broader_results = self.search_tool.optimized_search(broader_query, k=num_results)
...
class EnhancedReACTSignature(dspy.Signature):
    ...
    memory_context: str = dspy.InputField(desc="Relevant information from memory for this topic")
