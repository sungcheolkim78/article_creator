...
def display_article_metrics(article_data: Dict[str, Any]):
    ...
    with col3:
        st.metric(
            f"{st.session_state.generation_params['language']} Words",
            metrics["total_other_words"],
        )
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">📝 Enhanced Article Creator</h1>',
        unsafe_allow_html=True,
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["🚀 Article Generation", "📖 Article Review", "💾 Save & Export"]
    )

    with tab1:
        st.markdown(
            '<h2 class="sub-header">Article Generation</h2>', unsafe_allow_html=True
        )
        ...
        if st.button("🚀 Generate Article", type="primary", use_container_width=True):
            if not topic.strip():
                ...
                if article_data:
                    ...
                    st.session_state.generation_time = generation_params[
                        "generation_time"
                    ]
    ...
    with tab2:
        st.markdown(
            '<h2 class="sub-header">Article Review</h2>', unsafe_allow_html=True
        )
        ...
        if st.session_state.article_generated and st.session_state.current_article:
            ...
            with st.expander("📊 Generation Information", expanded=False):
                ...
                st.write(
                    f"**ReACT Agent:** {'Enabled' if params['use_react'] else 'Disabled'}"
                )
                st.write(f"**Generated At:** {params['generation_time']}")
        else:
            st.info(
                "📝 No article generated yet. Please generate an article first in the 'Article Generation' tab."
            )
    ...
    with tab3:
        ...
        if st.session_state.article_generated and st.session_state.current_article:
            ...
            if st.button(
                "💾 Save Article to Files", type="primary", use_container_width=True
            ):
